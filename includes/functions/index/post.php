  ...
  if ($db->num_rows("SELECT * FROM `post` WHERE `post_id` = {$id}") > 0) {
    ...
    if ($url != $cms['url']) {
      ...
      if ($post['status'] == 'public' || ($post['status'] != 'public' && !empty($ss->get("logined")))) {
        ...
        $post['url'] = rewrite_url($post['name'] . '-' . $post['post_id']);
        ...
        $post['has_like'] = false;
        if ($db->num_rows("SELECT * FROM `like` WHERE `post_id` = {$post['post_id']} AND `ip` = '".ip()."' ") > 0) {
          $post['has_like'] = true;
        }
        ...
        $post['comment'] = $db->fetch_assoc("SELECT * FROM `comment` WHERE `post_id` = {$post['post_id']} AND `accept` = TRUE ORDER BY `time` DESC");
        ...
        if (!empty($ss->get("logined"))) {
          $script_array[] = '$.shareButtons("admin-post", {';
          $script_array[] = 'position: "left",';
          $script_array[] = 'buttons: {';
          $script_array[] = '"edit": {';
          $script_array[] = 'class: "edit bg-' . $theme . '",';
          $script_array[] = 'icon: "fas fa-edit",';
          $script_array[] = 'link: "/admin/edit-post.' . $post['post_id'] . '.html"';
          $script_array[] = '},';
          $script_array[] = '"delete": {';
          $script_array[] = 'class: "delete bg-' . $theme . '",';
          $script_array[] = 'icon: "fas fa-trash-alt",';
          $script_array[] = 'link: "/admin/delete-post.' . $post['post_id'] . '.html"';
          $script_array[] = '}';
          $script_array[] = '}';
          $script_array[] = '});';
        }
